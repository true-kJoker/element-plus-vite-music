...
export async function useLogin(phone: string, password: string, captcha: string) {
...
export async function usePlayListTrackAll(id: number, limit: number, offset: number) {
...
export async function useAlbumNew(params?: { offset: number; limit: number; area: string }) {
    return await http.get<{
        albums: Album[]
        code: number
        total: number
